...
import com.flaminiovilla.geopic.repository.ReferralPersonRepository;
    ...
    public ReferralPerson create(ReferralPersonDTO referralPersonDTO) {
        ...
        if (secretaryHelper.existById(referralPersonDTO.secretaryId)) {
            if (!referralPersonRepository.existsByEmail(referralPersonDTO.email)) {
                ...
                // l'email esiste gia
                throw new ReferralPersonException(ReferralPersonException.referralPersonExceptionCode.REFERRAL_PERSON_ALREADY_EXIST);
            }
        } else {
            //la secretary non esiste
            throw new ReferralPersonException(ReferralPersonException.referralPersonExceptionCode.SECRETARY_NOT_EXIST);
        }
    }
    ...
    public Boolean delete(ReferralPersonDTO referralPersonDTO) {
        if (deletePermit(referralPersonDTO)) {
            try {
                ...
                throw new ReferralPersonException(ReferralPersonException.referralPersonExceptionCode.REFERRAL_PERSON_DELETE_ERROR);
            }
        }
